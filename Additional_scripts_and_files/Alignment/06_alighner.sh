...
NUM_BATCHES=$(( ( ${#SAMPLE_NAMES[@]} + BATCH_SIZE - 1 ) / BATCH_SIZE ))
  ...
  for ((j = START_INDEX; j <= END_INDEX && j < ${#SAMPLE_NAMES[@]}; j++)); do
    ...
    echo "STAR --runThreadN 4 --genomeDir /netscratch/dep_tsiantis/grp_laurent/tamal/2023/QC_Library/hg38/refgenome_index --readFilesIn \"\${R1}\" \"\${R2}\" --outFileNamePrefix \"${OUTPUT_DIR}/${SAMPLE}_alignment_\"" >> "$JOB_SCRIPT"
  done
