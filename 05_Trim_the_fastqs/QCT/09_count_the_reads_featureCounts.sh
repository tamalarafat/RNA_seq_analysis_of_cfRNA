...
mkdir "/netscratch/dep_tsiantis/grp_laurent/tamal/2023/QC_Library/Counts_aligned_data"
...
NUM_BATCHES=$(( ( ${#SAMPLE_NAMES[@]} + BATCH_SIZE - 1 ) / BATCH_SIZE ))
  ...
  for ((j = START_INDEX; j <= END_INDEX && j < ${#SAMPLE_NAMES[@]}; j++)); do
    ...
    echo "featureCounts -T 5 -p --countReadPairs -t exon -g gene_id -a \"${ANNOTATION_FILE}\" -o \"${O1}\" \"${I1}\"" >> "$JOB_SCRIPT"
    ...
    # job submission part
    bsub -q multicore20 -n 10 -R 'rusage[mem=30000]' -M 300000 -o CR_SA_out -e CR_SA_error.log -J CR_SA "$JOB_SCRIPT"
